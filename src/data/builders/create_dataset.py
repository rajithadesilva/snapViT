...
def process_real_data(args):
    ...
    # Temp extraction folders (removed at the end)
    ugv_temp_rgb = os.path.join(args.output_dir, "temp_ugv_extracted_images")
    ugv_temp_depth = os.path.join(args.output_dir, "temp_ugv_extracted_depths")
    drone_temp_folder = os.path.join(args.output_dir, "temp_drone_tiles")

    # Global de-duplicated storage (kept)
    global_rgb_dir = os.path.join(args.output_dir, "ugv_rgb")
    global_depth_dir = os.path.join(args.output_dir, "ugv_depth")
    os.makedirs(global_rgb_dir, exist_ok=True)
    os.makedirs(global_depth_dir, exist_ok=True)
    ...
    # 3) Extract UGV RGB+Depth
    ugv_frames, ugv_intrinsics = extract_frames_from_bag(
        args.ground_rosbag, ugv_temp_rgb, ugv_temp_depth, args.frame_skip
    )
    ...
    for ugv_info in tqdm(ugv_synced, desc="Assigning UGV images to scenes"):
        ...
        for scene_idx, d_xy in enumerate(drone_xy):
            if abs(d_xy[0] - ugv_x) <= args.tile_ground_size/2 and abs(d_xy[1] - ugv_y) <= args.tile_ground_size/2:
               scenes[scene_idx].append(ugv_info)
    ...
    # 7) Build dataset (NO per-scene UGV copying)
    ...
    for drone_idx, ugv_list in scenes.items():
        ...
        # Copy UAV image/tile into the scene folder
        ...
        if args.tile_resizing is not None:
            uav_img = Image.open(os.path.join(uav_out_path, uav_fname))
            uav_img = uav_img.resize((args.tile_resizing, args.tile_resizing), Image.BILINEAR)
            # Remove "original_" prefix if present
            resized_fname = uav_fname.replace("original_", "", 1)
            uav_img.save(os.path.join(uav_out_path, resized_fname))
        ...
        for ugv_info in ugv_list:
            ...
            dep_src = os.path.join(ugv_temp_depth, ugv_info["name"])
            if not os.path.exists(rgb_src) or not os.path.exists(dep_src):
                continue

            # Copy once into global shared folders (skip if already there)
            global_rgb_dst = os.path.join(global_rgb_dir, ugv_info["name"])
            global_depth_dst = os.path.join(global_depth_dir, ugv_info["name"])

            if not os.path.exists(global_rgb_dst):
                shutil.copy(rgb_src, global_rgb_dst)
            if not os.path.exists(global_depth_dst):
                shutil.copy(dep_src, global_depth_dst)
            ...
            if drone_info.get("tile_angle_deg") is not None:
                ...
                # rotate position into tile frame
                t_xy = np.array([local_x, local_y])
                t_xy = R_tile_inv[:2, :2] @ t_xy
                local_x, local_y = t_xy

                # rotate orientation into tile frame
                rotation_matrix = R_tile_inv @ rotation_matrix
            ...
            ugv_metadata.append(
                {
                    ...
                    "image_path": os.path.join("ugv_rgb", ugv_info["name"]),
                    "depth_path": os.path.join("ugv_depth", ugv_info["name"]),
    ...
    # Cleanup temp folders only
    if os.path.exists(ugv_temp_rgb):
        print(f"Cleaning up temporary directory: {ugv_temp_rgb}")
        shutil.rmtree(ugv_temp_rgb)
    if os.path.exists(ugv_temp_depth):
        print(f"Cleaning up temporary depth directory: {ugv_temp_depth}")
...
if __name__ == "__main__":
    ...
    drone_source_group.add_argument(
        "--drone_folder",
        type=str,
        default="data/aerial39",
        help="Path to the folder with geotagged drone images.",
    )
    drone_source_group.add_argument(
        "--drone_geotiff",
        type=str,
        default="data/odm_orthophoto.tif",
        help="Path to a single GeoTIFF orthophoto to be tiled.",
    )

    parser.add_argument("--ground_rosbag", type=str, default="data/row_1_to_6.bag", help="Path to Realsense .bag.")
    parser.add_argument("--ground_llh", type=str, default="data/1_6.LLH", help="Path to .llh GPS file.")
    parser.add_argument("--output_dir", type=str, default="datasets/dataset2", help="Output directory.")
    ...
    parser.add_argument("--tile_ground_size", type=float, default=10.0, help="Tile width and height in metres.")
    parser.add_argument('--tile_resizing', type=int, default=512, help="Resize UAV images to this size (square).")

    # Uniform tiling controls
    parser.add_argument("--tile_count", type=int, default=20, help="Aerial tile count to attempt.")
    parser.add_argument("--tile_rot_deg", type=float, default=00.0, help="Max abs rotation for tiles (± degrees).")
    parser.add_argument("--tile_seed", type=int, default=0, help="Random seed for tile rotations.")
    parser.add_argument("--tile_padding_factor", type=float, default=1.6, help="Padding factor when reading tiles.")
